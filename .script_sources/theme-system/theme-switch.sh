...
switch_wezterm() {
    ...
    # Update config.colors line in wezterm.lua (your config format)
    if grep -q 'config\.colors.*=.*Theme\.scheme\[' "$wezterm_lua" 2>/dev/null; then
        if [ "$theme" = "dark" ]; then
            sed -i.bak 's/config\.colors = lightTheme\.scheme\["Soft Focus Light"\]/config.colors = darkTheme.scheme["Soft Focus Dark"]/' "$wezterm_lua"
        else
            sed -i.bak 's/config\.colors = darkTheme\.scheme\["Soft Focus Dark"\]/config.colors = lightTheme.scheme["Soft Focus Light"]/' "$wezterm_lua"
        fi
        rm -f "${wezterm_lua}.bak"
        print_success "WezTerm theme updated"
    elif grep -q 'color_scheme.*=.*"Soft Focus' "$wezterm_lua" 2>/dev/null; then
        # Fallback for color_scheme format
        if [ "$theme" = "dark" ]; then
        ...
        # Capitalize first letter (compatible with bash 3.2)
        theme_capitalized="$(echo "${theme:0:1}" | tr '[:lower:]' '[:upper:]')${theme:1}"
        print_info "WezTerm: Add theme configuration to wezterm.lua"
    fi

    # WezTerm auto-reloads config
...
switch_vscode() {
    ...
    if grep -q '"workbench.colorTheme":' "$vscode_settings" 2>/dev/null; then
        if [ "$theme" = "dark" ]; then
            sed -i.bak 's/"workbench.colorTheme": *"Soft Focus Light"/"workbench.colorTheme": "Soft Focus Dark"/' "$vscode_settings"
        else
            sed -i.bak 's/"workbench.colorTheme": *"Soft Focus Dark"/"workbench.colorTheme": "Soft Focus Light"/' "$vscode_settings"
        fi
        rm -f "${vscode_settings}.bak"
        print_success "VS Code theme updated"
        print_info "VS Code: Reload window to see changes"
    fi
}
...
switch_aerospace() {
    ...
    # Check if Aerospace is installed and running
    if ! command -v aerospace &> /dev/null; then
        return
    fi

    # Aerospace doesn't require theme file changes as it uses system colors
    # But we can trigger a reload to ensure it picks up system appearance changes
    if pgrep -x AeroSpace > /dev/null 2>&1; then
        aerospace reload-config 2>/dev/null && print_success "Aerospace configuration reloaded" || print_info "Aerospace: Manual reload may be required"
    else
        print_info "Aerospace: Not currently running"
    fi
}
...
switch_theme() {
    ...
    # macOS system appearance and window manager
    if [ "$(detect_os)" = "macos" ]; then
        switch_macos_appearance "$target_theme"
        switch_aerospace "$target_theme"
    fi
